...
# example:
cat integrated_call_samples_v3.20130502.ALL.panel | grep EUR | cut -f1 > integrated_call_samples_v3.20130502.EUR.panel

# Calculate FST and Tajima's D on Chr 2 for EUR, AFR and EAS:
INFILE=ALL.chr2.phase3_shapeit2_mvncall_integrated_v5a.20130502.genotypes.vcf.gz
EURcodes=integrated_call_samples_v3.20130502.EUR.panel.txt
EAScodes=integrated_call_samples_v3.20130502.EAS.panel.txt

# Keep SNPs only
#-----------------------------------
vcftools --gzvcf ${INFILE} --remove-indels --recode --recode-INFO-all --out EMBOrun01_chr2SNPs_only
echo "--remove-indels ended:"
date

# Calculate FST EUR-EAS
#-----------------------------------
...
# biallelic sites, coded 0/1, no missing and phased data
# NOTE: we use vcf and map files, but other input files can be used with selscan 

# 1. Generate the vcf files
#-----------------------------------
# Use vcflib and bgzip and selscan
# Download VCFLIB
# Follow instructions to download and make, from 
# https://github.com/vcflib/vcflib

# uncompress the file and zip it using bgzip
# NOTE: bgzip is part of htslib 
...
#--> check ENSMBL genome assembly and rs number location on the chromosome 

# whole region for selscan, I use vcflib to filter based on quality
#vcffilter -f "VT = SNP" -f "AC > 1" -f "QUAL > 20" -f "DP > 40" -f "DP < 40000" -r $CHROM:$START-$END $VCF > ALL.chr$CHROM.LCT.tmp.vcf
...
# Keep only some samples of interest
#cat /Users/gerbaup/Documents/workshop/DATA/integrated_call_samples_v3.20130502.ALL.panel.txt | grep CHB | cut -f1 > /Users/gerbaup/Documents/workshop/DATA/integrated_call_samples_v3.20130502.CHB.panel.txt
...
cp CHB.chr2.LCT.tmp.vcf CHB.chr2.LCT.vcf
# check with bcftools
...
# 2. The recombination map:
#-----------------------------------
# Download recombination rate maps for population of interest from:
# ftp://ftp.1000genomes.ebi.ac.uk/vol1/ftp/technical/working/20130507_omni_recombination_rates/
# or on linux/unix commands:
#wget ftp://ftp.1000genomes.ebi.ac.uk/vol1/ftp/technical/working/20130507_omni_recombination_rates/GBR_omni_recombination_20130507.tar
...
#wget ftp://ftp.1000genomes.ebi.ac.uk/vol1/ftp/technical/working/20130507_omni_recombination_rates/YRI_omni_recombination_20130507.tar
# example with CHB
mkdir CHBMap
...
#------------
# NOTE: this R code is from M. Fumagalli
# fmap="CHBgenetic_map_chrom2.map"
# fvcf="CHB.chr2.LCT.vcf"
...
norm --iHS --files CHB.LCT.ihs.ihs.out 
